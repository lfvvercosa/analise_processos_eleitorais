...
def create_1_gram_features(df, min_perc, max_perc, n):
    id_col = 'case:concept:name'
    activity_col = 'concept:name'

    df_gram = my_cluster.create_n_gram(df, 
                                       id_col, 
                                       activity_col,
                                       n)

    rem_cols = selectColsByFreq(min_perc, max_perc, df_gram)
...
def get_attribute_level(breadscrum, level):
...
def get_total_movs_first_level(df, df_code_mov):
    level = 0
    df_code = df_code_mov[['movimentoCodigoNacional','breadscrum']]
    df_code = df_code.rename(columns={'movimentoCodigoNacional':'concept:name'})
    df_code['first_level'] = df_code.apply(lambda df: \
                                      get_attribute_level(
                                        df['breadscrum'],
                                        level), axis=1)
    df_code = df_code.drop(columns='breadscrum')
    df_code['concept:name'] = df_code['concept:name'].astype(str)

    df_work = df[['case:concept:name','concept:name']]
    df_work = df_work.merge(df_code, on='concept:name',how='left')

    df_work['total_magistrate'] = (df_work['first_level'] == 1).astype(int)
    df_work['total_official'] = (df_work['first_level'] == 14).astype(int)
    
    df_work = df_work.groupby('case:concept:name').\
        agg(total_magistrate=('total_magistrate','sum'),
            total_official=('total_official','sum'))
...
def get_df_assu(df):
    df_assu = df[['case:concept:name','case:lawsuit:subjects']]
    df_assu['case:lawsuit:subjects'] = df_assu.apply(lambda df: \
                                      convert_to_list(
                                      df['case:lawsuit:subjects']), 
                                      axis=1)
    df_assu = df_assu.explode('case:lawsuit:subjects')
    df_assu = df_assu.dropna()
